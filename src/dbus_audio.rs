...
fn default_handler(_recording_state_ctx: &mut RecordingState, _matches: Matches, _msg: &MarshalledMessage, _env: &mut MyHandleEnv) -> HandleResult<()> {
    Ok(None)
}

fn audio_handler(recording_state_ctx: &mut RecordingState, _matches: Matches, msg: &MarshalledMessage, _env: &mut MyHandleEnv) -> HandleResult<()> {
...
pub fn setup_dbus_test_audio_recording_service(recording_state: &RecordingState) {
    ...
    let _dbus_thread = thread::Builder::new().name("dbus-service".to_string()).spawn_with_priority(ThreadPriority::Max, move |_| {
        ...
        let mut dispatch_conn = DispatchConn::new(dbus_conn, recording_state, Box::new(default_handler));
